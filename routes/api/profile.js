...
router.post('/', [auth, [
    ...
    async (req, res) => {
        ...
        if (skills) {
            // turn to array with split, by delimiter of comma/space and map function
        ...
        try {
            // note that user field is the objectId
...
router.get('/user/:user_id', async (req, res) => {
    try {
        ...
        if (!profile) return res.status(400).json({ msg: 'There is no record found \n... you son of a bitch' })
        res.json(profile);
    } catch (err) {
        console.error(err.message);
        if (err.kind == 'ObjectId') { return res.status(400).json({ msg: 'There is no record found \n... you son of a bitch' }) }
        res.status(500).send('... Server is @#(burp)$%ed')
    }
});
...
// @access  Public
// @todo add authentication to github route
